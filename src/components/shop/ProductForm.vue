<template>
  <form @submit.prevent="saveProduct" class="p-5 bg-white">
    <div class="row">
        ...
        <div class="form-group">
          ...
          >
            <template v-for="manufacturer in manufacturers">
              <option :key="manufacturer.id" :value="manufacturer.id" :selected="manufacturer.id == (model.manufacturer && model.manufacturer.id)">{{manufacturer.name}}</option>
            </template>
          </select>
